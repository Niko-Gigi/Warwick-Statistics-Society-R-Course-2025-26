...
This chapter covers two powerful techniques for displaying multiple variables:
...
Here's a comprehensive example using multiple techniques:

```{r}
#| warning: false
#| message: false
# Create a rich, multi-variable visualization
ggplot(penguins, aes(x = flipper_length_mm, 
                     y = body_mass_g,
                     color = species,
                     shape = sex)) +
...
5. Challenge: Recreate the final "putting it all together" example with different variables

# Summary

You've learned to:

- Add multiple variables using color, shape, size, and transparency
- Create faceted plots with `facet_wrap()` and `facet_grid()`
- Combine grouping and faceting for complex visualizations
- Choose the right approach for your data and question
- Avoid common pitfalls in multivariate visualization

These techniques let you explore complex datasets and communicate sophisticated insights. The key is finding the right balance between showing enough detail and keeping your visualizations clear and interpretable.
